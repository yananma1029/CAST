...
dataset = Dataset()
dataset.load_custom_dataset_from_folder("octis_dataset/Twitter_Young")
corpus = dataset.get_corpus()
documents = [" ".join(words) for words in corpus]
...
for self_sim_threshold in [0.3]:
    ...
    with open(f"Twitter_Young_{self_sim_threshold}_Contextualized_{MODEL_NAME.split('/')[1]}_{10}.json", "w") as file:
